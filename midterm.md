---
layout: page
title: Midterm
nav_order: 2
parent: Deliverables
description: Midterm project details and resources.
---

# Midterm

For the midterm project, each student will improve a computational biology article for the ISCB Wikipedia competition. Although the external competition remains open until May, the course project must be completed during the fall semester.

## Milestones

| Due date | Milestone |
|:---|:---|
| **Monday, October 5** | **Topic proposal** — propose an article for instructor approval; do not register it yet. |
| **Friday, October 9** | **Approved topic claimed** — after receiving instructor approval, register the article on the competition page. |
| **Friday, October 16** | **Existing-article analysis** — identify strengths, weaknesses, gaps, and sourcing needs. |
| **Friday, October 30** | **Draft** — complete the proposed edits with sources and edit summaries in place. |
| **Friday, November 6** | **Peer review** — give and receive actionable feedback on the draft asynchronously. |
| **Friday, November 13** | **Final Wikipedia submission** — publish the revised article and submit the accompanying reflection. |

For the final submission, the revised article must be published on Wikipedia, with complete sources and edit summaries, and the accompanying reflection must be submitted. Competition administration may occur later, but the academic work for this project must be finished by November 13.

## Competition Entries

[View competition entries and claim a topic](https://en.wikipedia.org/wiki/Wikipedia:WikiProject_Molecular_Biology/Computational_Biology/16th_ISCB_Wikipedia_competition_entries){: .btn .btn-primary }

[View the WP1 Computational Biology project](https://wp1.openzim.org/#/project/Computational%20Biology){: .btn .btn-primary }

## Example

[View an example revision to the National Center for Biotechnology Information article](https://en.wikipedia.org/w/index.php?title=National_Center_for_Biotechnology_Information&diff=1352527190&oldid=1349178304)
